...
public class Activity2 {
	...
	public void getUserInfo() {
		...
		Response response = given()
				...
				.pathParam("username", "vidhu")// Pass request body from file
		...
		response.then().body("username", equalTo("vidhu"));
	...
	public void deleteUser() throws IOException {
		Response response = given()
			.baseUri("https://petstore.swagger.io/v2/user") // Set base URI
			.header("Content-Type", "application/json") // Set headers
			.pathParam("username", "vidhu") // Add path parameter
			.when().delete("/{username}"); // Send POST request
 
		// Assertion
		response.then().body("code", equalTo(200));
		response.then().body("message", equalTo("vidhu"));
	}
